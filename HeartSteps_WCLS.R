### WCLS on 30-min aggregate outcome. Analysis 40-min and 60-min can be done by replacing 29 by 39 and 59 in line 30.
rm(list = ls())
library(tidyverse)
library(gridExtra)
library(ggpubr)
...
dta$is.weekday.num <- ifelse(dta$is.weekday == TRUE, 1, 0) #weekday = 1
dta$homework.num <- ifelse(dta$location.other == FALSE, 1, 0) #home/work = 1
dta$jbsteps30pre.zero.bin_56 <- as.factor(ifelse(dta$jbsteps30pre.zero < 56, TRUE, FALSE)) #sedentary = 1
dta$jbsteps30pre.zero.bin_56.num <- ifelse(dta$jbsteps30pre.zero.bin_56 == TRUE, 1, 0)
...
dta_logmin <- dta %>% 
  ...
  filter(min.after.decision <= 29) %>%  # 30-min aggregate outcome
  summarise(totalsteps_logmins = log(sum(steps) + 0.5))
dta <- merge(dta, dta_logmin, by = c("user", "decision.index.nogap"))
...
dat_agg <- dat %>% dplyr::select(user, decision.index.nogap, avail, send.active, send.sedentary,
                                 jbsteps30pre.log, 
                                 homework.num, 
                                 is.weekday, is.weekday.num, 
                                 jbsteps30pre.zero.bin_56, jbsteps30pre.zero.bin_56.num,
                                 totalsteps_logmins)
dat_agg$"(Intercept)" <- 1
dat_agg$"I(send.active - 0.3)" <- dat_agg$send.active - 0.3
dat_agg$"I(send.sedentary - 0.3)" <- dat_agg$send.sedentary - 0.3
dat_agg <- dat_agg[order(dat_agg$user, dat_agg$decision.index.nogap),]

gee_printresult <- function(fit){
  tab <- as.data.frame(tab)
  # round and coloring
  tab_round <- round(tab, 3)
  col_pval <- which(str_detect(colnames(tab_round), "p-value"))
  col_est <- which(str_detect(colnames(tab_round), "Estimate"))
  tab_round[, col_est] <-  cell_spec(tab_round[, col_est], color = ifelse(tab[, col_pval] < 0.05, "red","black"))
  tab_round <- tab_round[, c("Estimate", "SE", "p-value")]
  tab_round
}

## Functions of xgeepack:
...
### 1.1 Oveall, no linear time trend (the [1] analysis in Klasnja et al.(2019))
xmat <- dat_agg %>%
  ungroup() %>%
  transmute("(Intercept)" = .$"(Intercept)",
            "jbsteps30pre.log" = .$"jbsteps30pre.log",
            "decision.index.nogap" = .$"decision.index.nogap",
            "homework.num" = .$"homework.num",
            "is.weekday.num" = .$"is.weekday.num",
            "jbsteps30pre.zero.bin_56.num" = .$"jbsteps30pre.zero.bin_56.num",
            "I(send.active - 0.3)" = .$"I(send.active - 0.3)",
            "I(send.sedentary - 0.3)" = .$"I(send.sedentary - 0.3)")

fit_nomod <- geese.glm(x = as.matrix(xmat), 
                       y = dat_agg$totalsteps_logmins, 
                       w = dat_agg$avail, 
                       id = as.factor(dat_agg$user), family = gaussian(), corstr = "independence")
estimate(fit_nomod)


### 1.2 Linear time trend (the [2] analysis in Klasnja et al.(2019))
xmat <- dat_agg %>%
  ungroup() %>%
  transmute("(Intercept)" = .$"(Intercept)",
            "jbsteps30pre.log" = .$"jbsteps30pre.log",
            ...
            "jbsteps30pre.zero.bin_56.num" = .$"jbsteps30pre.zero.bin_56.num",
            "I(send.active - 0.3)" = .$"I(send.active - 0.3)",
            "I(send.active - 0.3):decision.index.nogap" = .$"I(send.active - 0.3)"*.$"decision.index.nogap",
            "I(send.sedentary - 0.3)" = .$"I(send.sedentary - 0.3)",
            "I(send.sedentary - 0.3):decision.index.nogap" = .$"I(send.sedentary - 0.3)"*.$"decision.index.nogap")
...
xmat <- dat_agg %>%
  ...
  transmute("(Intercept)" = .$"(Intercept)",
            ...
            "I(send.active - 0.3)" = .$"I(send.active - 0.3)",
            "I(send.active - 0.3):decision.index.nogap" = .$"I(send.active - 0.3)"*.$"decision.index.nogap",
            "I(send.active - 0.3):homework.num" = .$"I(send.active - 0.3)"*.$"homework.num",
            "I(send.sedentary - 0.3)" = .$"I(send.sedentary - 0.3)",
            "I(send.sedentary - 0.3):decision.index.nogap" = .$"I(send.sedentary - 0.3)"*.$"decision.index.nogap",
            "I(send.sedentary - 0.3):homework.num" = .$"I(send.sedentary - 0.3)"*.$"homework.num")

fit_loc <- geese.glm(x = as.matrix(xmat), 
                     y = dat_agg$totalsteps_logmins, 
                     w = dat_agg$avail, 
                     id = as.factor(dat_agg$user), family = gaussian(), corstr = "independence")
estimate(fit_loc)
...
xmat <- dat_agg %>%
  ...
  transmute("(Intercept)" = .$"(Intercept)",
            ...
            "I(send.active - 0.3):decision.index.nogap" = .$"I(send.active - 0.3)"*.$"decision.index.nogap",
            "I(send.active - 0.3):homework.num" = .$"I(send.active - 0.3)"*.$"homework.num",
            "I(send.active - 0.3):decision.index.nogap:homework.num" = .$"I(send.active - 0.3)"*.$"decision.index.nogap"*.$"homework.num",
            "I(send.sedentary - 0.3)" = .$"I(send.sedentary - 0.3)",
            "I(send.sedentary - 0.3):decision.index.nogap" = .$"I(send.sedentary - 0.3)"*.$"decision.index.nogap",
            "I(send.sedentary - 0.3):homework.num" = .$"I(send.sedentary - 0.3)"*.$"homework.num",
            "I(send.sedentary - 0.3):decision.index.nogap:homework.num" = .$"I(send.sedentary - 0.3)"*.$"decision.index.nogap"*.$"homework.num")

fit_loc <- geese.glm(x = as.matrix(xmat), 
                     y = dat_agg$totalsteps_logmins, 
                     w = dat_agg$avail, 
                     id = as.factor(dat_agg$user), family = gaussian(), corstr = "independence")
tab <- estimate(fit_loc)
kable(gee_printresult(tab), escape=FALSE) %>% 
...
xmat <- dat_agg %>%
  ...
  transmute("(Intercept)" = .$"(Intercept)",
            ...
            "I(send.active - 0.3)" = .$"I(send.active - 0.3)",
            "I(send.active - 0.3):decision.index.nogap" = .$"I(send.active - 0.3)"*.$"decision.index.nogap",
            "I(send.active - 0.3):is.weekday.num" = .$"I(send.active - 0.3)"*.$"is.weekday.num",
            "I(send.sedentary - 0.3)" = .$"I(send.sedentary - 0.3)",
            "I(send.sedentary - 0.3):decision.index.nogap" = .$"I(send.sedentary - 0.3)"*.$"decision.index.nogap",
            "I(send.sedentary - 0.3):is.weekday.num" = .$"I(send.sedentary - 0.3)"*.$"is.weekday.num")
...
xmat <- dat_agg %>%
  ...
  transmute("(Intercept)" = .$"(Intercept)",
            ...
            "I(send.active - 0.3):decision.index.nogap" = .$"I(send.active - 0.3)"*.$"decision.index.nogap",
            "I(send.active - 0.3):is.weekday.num" = .$"I(send.active - 0.3)"*.$"is.weekday.num",
            "I(send.active - 0.3):decision.index.nogap:is.weekday.num" = .$"I(send.active - 0.3)"*.$"decision.index.nogap"*.$"is.weekday.num",
            "I(send.sedentary - 0.3)" = .$"I(send.sedentary - 0.3)",
            "I(send.sedentary - 0.3):decision.index.nogap" = .$"I(send.sedentary - 0.3)"*.$"decision.index.nogap",
            "I(send.sedentary - 0.3):is.weekday.num" = .$"I(send.sedentary - 0.3)"*.$"is.weekday.num",
            "I(send.sedentary - 0.3):decision.index.nogap:is.weekday.num" = .$"I(send.sedentary - 0.3)"*.$"decision.index.nogap"*.$"is.weekday.num")
...
xmat <- dat_agg %>%
  ...
  transmute("(Intercept)" = .$"(Intercept)",
            ...
            "I(send.active - 0.3)" = .$"I(send.active - 0.3)",
            "I(send.active - 0.3):decision.index.nogap" = .$"I(send.active - 0.3)"*.$"decision.index.nogap",
            "I(send.active - 0.3):jbsteps30pre.zero.bin_56.num" = .$"I(send.active - 0.3)"*.$"jbsteps30pre.zero.bin_56.num",
            "I(send.sedentary - 0.3)" = .$"I(send.sedentary - 0.3)",
            "I(send.sedentary - 0.3):decision.index.nogap" = .$"I(send.sedentary - 0.3)"*.$"decision.index.nogap",
            "I(send.sedentary - 0.3):jbsteps30pre.zero.bin_56.num" = .$"I(send.sedentary - 0.3)"*.$"jbsteps30pre.zero.bin_56.num")
...
xmat <- dat_agg %>%
  ...
  transmute("(Intercept)" = .$"(Intercept)",
            ...
            "I(send.active - 0.3):decision.index.nogap" = .$"I(send.active - 0.3)"*.$"decision.index.nogap",
            "I(send.active - 0.3):jbsteps30pre.zero.bin_56.num" = .$"I(send.active - 0.3)"*.$"jbsteps30pre.zero.bin_56.num",
            "I(send.active - 0.3):decision.index.nogap:jbsteps30pre.zero.bin_56.num" = .$"I(send.active - 0.3)"*.$"decision.index.nogap"*.$"jbsteps30pre.zero.bin_56.num",
            "I(send.sedentary - 0.3)" = .$"I(send.sedentary - 0.3)",
            "I(send.sedentary - 0.3):decision.index.nogap" = .$"I(send.sedentary - 0.3)"*.$"decision.index.nogap",
            "I(send.sedentary - 0.3):jbsteps30pre.zero.bin_56.num" = .$"I(send.sedentary - 0.3)"*.$"jbsteps30pre.zero.bin_56.num",
            "I(send.sedentary - 0.3):decision.index.nogap:jbsteps30pre.zero.bin_56.num" = .$"I(send.sedentary - 0.3)"*.$"decision.index.nogap"*.$"jbsteps30pre.zero.bin_56.num")
