...
gen_data_discrete <- function(i_total, t_total, j_total, gen_formul, 
  ...
  for (i in 1:i_total){
    ...
    e_it <- rsn(j_total*t_total, 0, 2, 5)
    e_it <- e_it - mean(e_it)
    ...
    #add random effect: random intercept
    b0_i_trt <- rnorm(1, mean = 0, sd = sqrt(0.3))
    b0_i_nuis <- rnorm(1, mean = 0, sd = sqrt(0.3))
  ...
  b0_i_trt <- dta$b0_i_trt
  b0_i_nuis <- dta$b0_i_nuis
